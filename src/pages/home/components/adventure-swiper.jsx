...
function AdventureSwiper() {
  const [movies, setMovies] = useState([]);
  const swiperRef = useRef(null); // Reference to the Swiper instance

  // Fetch adventure movies from the API
  const fetchMovies = async () => {
    try {
      const response = await fetch(
        "https://yts.mx/api/v2/list_movies.json?genre=adventure&limit=10"
      );
  ...
  return (
    <section className="my-16 mx-48">
      <h2 className="text-3xl text-white font-bold mb-6">Adventure Movies</h2>
      ...
      <Swiper
        ...
        className="adventure-movie-swiper"
      >
        {movies.map((movie) => (
...
export default AdventureSwiper;
